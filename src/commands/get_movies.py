...
def build_keyboard(pagination_metadata) -> InlineKeyboardMarkup:
    ...
    if pagination_metadata["links"]["previous"]:
        buttons.append(
            InlineKeyboardButton(
                "◀️ Anterior",
                callback_data=f"pagination:{pagination_metadata['links']['previous']}",
            )
        )
    if pagination_metadata["links"]["next"]:
        buttons.append(
            InlineKeyboardButton(
                "▶️ Siguiente",
                callback_data=f"pagination:{pagination_metadata['links']['next']}",
            )
        )
...
async def handle_pagination(update, context):
    ...
    endpoint = query.data.replace("pagination:", "")

    metadata, movies = await get_movies_from_api(endpoint)
    text = create_reply_text(movies, metadata)
...
handle_pagination_handler = CallbackQueryHandler(
    handle_pagination, pattern="^pagination:"
)
